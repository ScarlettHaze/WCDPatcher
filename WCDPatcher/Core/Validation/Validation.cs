           ...
           if (DoByteCheck)
           {
               ...
               else
               {
                   ...
                   if (ErrorDialogs.WonderCardIsInvalid(wonderCardPath, System.Windows.Forms.MessageBoxButtons.YesNo,
                       false, "Continue anyway, ignoring the byte inconsistencies?") == System.Windows.Forms.DialogResult.No)
                   { return false; }
               }
           }
           return true;
               ...
               if (CRCCheck)
               {
                   ...
                   if (!result)
                   {
                       ...
                       if (ErrorDialogs.NDSFileIsInvalid(ndsPath, System.Windows.Forms.MessageBoxButtons.YesNo,
                           false, "The CRC error check failed. Would you like to continue anyway?") == System.Windows.Forms.DialogResult.Yes)
                       {
                           return true;
                       }
                       else return false;
